...
const GameCoreModule = (function () {
  ...
  function handleSymbolClick(event) {
    ...
    if (StateModule.getGameState().selectedLeftSymbol && StateModule.getGameState().selectedRightSymbol) {
      checkMatch();
    }
  }
  ...
  function checkMatch() {
    ...
    if (isCorrect) {
      ...
      commonSymbol = leftSymbols.find((symbol) => rightSymbols.includes(symbol));
    ...
    setTimeout(() => {
      ...
      if (isCorrect) {
        ...
        HintsCoreModule.DeckModule.moveCardToUsedDeck(StateModule.getGameState().leftCardIndex);
        displayNextPair();
      } else {
        TimerWindowModule.resetPairTimer();
        if (
